<template>
  <div class="ecs-cart-page">
    <div class="container">
      <EcsBread>
        <EcsBreadItem to="/">首页</EcsBreadItem>
        <EcsBreadItem>购物车</EcsBreadItem>
      </EcsBread>
      <div class="cart">
        <table>
          <thead>
          ...
          <tbody>
            ...
            <tr v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
              ...
              <td class="tc">
                <EcsNumbox @change="$event => updateCount(goods.skuId,$event)" :max="goods.stock" :modelValue="goods.count" />
              </td>
              <td class="tc">
                <p class="f16 red">
          ...
          <tbody v-if="$store.getters['cart/invalidList'].length">
            ...
            <tr v-for="goods in $store.getters['cart/invalidList']" :key="goods.skuId">
              <td><EcsCheckbox style="color: #eee" /></td>
              <td>
                <div class="goods">
                  <router-link :to="`/product/${goods.id}`"
                    ><img :src="goods.picture" alt=""
      ...
      <div class="action">
        <div class="batch">
          <EcsCheckbox @change="checkAll" :modelValue="$store.getters['cart/isCheckAll']"
            >全选</EcsCheckbox
          >
        ...
        <div class="total">
          ...
          <EcsButton type="primary">下单结算</EcsButton>
        </div>
      </div>
      <!-- 猜你喜欢 -->
      <GoodRelevant />
...
export default {
  ...
  setup() {
    ...
    const batchDeleteCart = (isClear) => {
      Confirm({ text: `是否确认批量删除${isClear ? '失效的' : ''}选中商品` })
        .then(() => {
          store.dispatch("cart/batchDeleteCart",isClear).then(() => {
            Message({ type: "success", text: `${isClear ? '清空失效' : '批量删除选中'}商品成功` });
          });
        })
        .catch((error) => {
          // console.log('取消');
