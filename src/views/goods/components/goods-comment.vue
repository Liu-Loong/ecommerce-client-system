<template>
  ...
  <!-- 分页组件 -->
  <EcsPagination @currentChange="changePager" :total="total" :pageSize="reqParams.pageSize" :currentPage="reqParams.page"/>
</template>
<script>
...
export default {
  ...
  setup() {
    ...
    findGoodsCommentInfo(route.params.id).then(data => {
      //设置数据之前，tags数组前需要追加 有图tag 全部评价tag
